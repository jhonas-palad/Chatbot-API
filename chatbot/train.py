...
class Trainer:
    ...
    def _run_epoch_sync(self, model: NeuralNet, num_epochs = 10000, learning_rate = 0.001):
        ...
        for epoch in range(num_epochs):
            for x, y in dataloader:
                ...
                x = x.to(self.device)
                y = y.to(dtype=torch.long).to(self.device)
...
async def train_from_db(intents, **train_params):
    chatbot = ChatBot('MyBot')
    trainer = Trainer(chatbot)
    
    data = await trainer.train_model(intents, **train_params)
    return data
